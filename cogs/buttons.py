...
class RedditMediaURL:
    ...
    async def convert(cls, ctx: Context, argument: str) -> Self:
        ...
        headers = {
            'User-Agent': 'Discord:Mercybot:v4.0 (by /u/Rapptz)',
        }
        await ctx.typing()
...
class SpoilerCache:
    ...
    def to_embed(self, bot: Mercybot) -> discord.Embed:
...
class Buttons(commands.Cog):
    ...
    def feedback_channel(self) -> Optional[discord.TextChannel]:
        guild = self.bot.get_guild(798222957113638955)
        if guild is None:
            return None
    ...
    def storage_channel(self) -> Optional[discord.TextChannel]:
        ...
        return guild.get_channel(798223188709474384)  # type: ignore
