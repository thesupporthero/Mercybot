...
class AutoModFlags(flags.BaseFlags):
    ...
    @flags.flag_value
    def edits(self) -> int:
        """Whether the server is broadcasting edit messages"""
        return 16
    @flags.flag_value
    def deletes(self) -> int:
        """Whether the server is broadcasting delete messages"""
        return 32
...
class ModConfig:
    ...
    id: int
    modlog_enable: Optional[int]
    modlog_chid: Optional[int]
    ...
    @classmethod
    def from_record(cls, record: Any, bot: Mercybot):
        self = cls()
    ...
    def requires_migration(self) -> bool:
        return self.broadcast_channel_id is not None and self.broadcast_webhook_url is None #add in modlog check

    @property
    def requires_modlog_migration(self) -> bool:
        return self.modlog_enable is not None
    ...
    @property
    def modlog_channel(self) -> Optional[discord.TextChannel]:
        guild = self.bot.get_guild(self.id)
        ...
        while self.role_id is not None:
            ...
            try:
                if action is GatekeeperRoleState.pending_add:
                    await add_role(
                        self.id, member_id, self.role_id, reason=f'Mod Gatekeeper is active since {self.started_at}'
                    )
                    ...
                    await remove_role(self.id, member_id, self.role_id, reason='Completed Mod Gatekeeper verification')
...
class MigrateJoinLogView(discord.ui.View):
    ...
    @discord.ui.button(label='Migrate', custom_id='migrate_mod_join_logs', style=discord.ButtonStyle.green)
...
class MigratemodlogView(discord.ui.View):
    def __init__(self, cog: Mod):
        super().__init__(timeout=None)
        self.cog: Mod = cog

    @discord.ui.button(label='Migrate', custom_id='migrate_mod_logs', style=discord.ButtonStyle.green)
    async def migrate(self, interaction: discord.Interaction, button: discord.ui.Button):
        assert interaction.message is not None
        await interaction.response.defer(ephemeral=True, thinking=True)
        ...
        try:
            ...
            await self.cog.migrate_automod_edits(interaction.user, interaction.channel, interaction.guild_id)
            await self.cog.migrate_automod_deletes(interaction.user, interaction.channel, interaction.guild_id)
            await self.cog.disable_modlog_enable(interaction.guild_id)  # type: ignore
        except RuntimeError as e:
            await interaction.followup.send(str(e), ephemeral=True)
        else:
            await interaction.message.edit(content=None, view=None)
            await interaction.followup.send('Successfully migrated to new logs!', ephemeral=True)
...
class GatekeeperRateLimitModal(discord.ui.Modal, title='Join Rate Trigger'):
    ...
    async def on_submit(self, interaction: discord.Interaction[Mercybot], /) -> None:
        try:
...
class GatekeeperChannelSelect(discord.ui.ChannelSelect['GatekeeperSetUpView']):
    ...
    async def callback(self, interaction: discord.Interaction[Mercybot]) -> Any:
...
class GatekeeperAlertMassbanButton(discord.ui.DynamicItem[discord.ui.Button], template='gatekeeper:alert:massban'):
    ...
    async def callback(self, interaction: discord.Interaction[Mercybot]):
...
class Mod(commands.Cog):
    ...
    def __init__(self, bot: Mercybot):
        ...
        self._modlog_migration_view = MigratemodlogView(self)
        bot.add_view(self._automod_migration_view)
        bot.add_view(self._modlog_migration_view)
    ...
    async def cog_unload(self) -> None:
        ...
        self._modlog_migration_view.stop()
    ...
    async def check_raid(
        ...
        for user in members:
            try:
                ...
                log.info('[MercyMod] Failed to ban %s (ID: %s) from server %s.', member, member.id, member.guild)
            else:
                log.info('[MercyMod] Banned %s (ID: %s) from server %s.', member, member.id, member.guild)
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        if not config.automod_flags.joins and not config.automod_flags.gatekeeper and config.modlog_enable == 0:
            return
        ...
        if config.requires_migration or config.requires_modlog_migration:
            if config.requires_migration:
                await self.suggest_automod_migration(config, e, guild_id)
            if config.requires_modlog_migration:
                await self.suggest_modlog_migration(config, e, guild_id)
            return
    ...
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent):
        ...
        if message is not None:
            e.set_author(name=str(author), icon_url=author.display_avatar)
            ...
            if message.attachments:
                ...
                try:
                 for attachment in attachments:  
                    ...
                    Send_log = await config.broadcast_webhook.send(file=f, embed=e)
                except discord.errors.HTTPException:
                    return
            else:
                Send_log = await config.broadcast_webhook.send(embed=e)
            e.color=(0xFFC6FF)
        else:
        ...
        if config.automod_flags.deletes:
            try:
                Send_log
    ...
    async def on_message_edit(self, before: discord.Message, after: discord.Message):
        ...
        if config is None:
            return
        if not config.automod_flags.edits and config.modlog_enable == 0:
            return
        ...
        if config.requires_migration or config.requires_modlog_migration:
            if config.requires_migration:
                await self.suggest_automod_migration(config, e, guild_id)
                log.info('Suggested automod migration for message edit in guild ID %s', guild_id)
            if config.requires_modlog_migration:
                await self.suggest_modlog_migration(config, e, guild_id)
                log.info('Suggested modlog migration for message edit in guild ID %s', guild_id)
            return
        if config.modlog_channel:
            try:
                await config.modlog_channel.send(embed=e)
    ...
    async def suggest_modlog_migration(self, config: ModConfig, embed: discord.Embed, guild_id: int) -> None:
        channel = config.modlog_channel
        ...
        msg = (
            '**Notice**\n\n'
            'To help with future updates, all logs have been simplified and moved to webhooks '
            'As a result, **migration needs to be done '
        ...
        try:
            await channel.send(embed=embed, content=msg, view=self._modlog_migration_view)
        except discord.Forbidden:
            pass
    ...
    @checks.is_mod()
    async def mercymod(self, ctx: GuildContext):
        """Show current Mercymod (automatic moderation/logging) behaviour on the server.
        ...
        if config is None:
            return await ctx.send('This server does not have Mercymod set up!')

        e = discord.Embed(title='Mercymod Information', colour=discord.Colour.blurple())
        if config.automod_flags.joins:
            ...
            if config.requires_migration or config.requires_modlog_migration:
                broadcast = (
                    ...
                    f'Run "{ctx.prefix}mercymod disable joins" followed by "{ctx.prefix}mercymod join {channel}" '
        ...
        if config.automod_flags.edits:
            edit = f'Enabled on <#{config.broadcast_channel_id}>'
        else:
            edit = 'Disabled'

        if config.automod_flags.deletes:
            delete = f'Enabled on <#{config.broadcast_channel_id}>'
        else:
            delete = 'Disabled'
        
        e.add_field(name='Join Logs', value=broadcast)
        e.add_field(name='Edit Logs', value=edit)
        e.add_field(name='Delete Logs', value=delete)
    ...
    @mercymod.command(name='enable', aliases=['on'])
    @checks.is_mod()
    @app_commands.describe(channel='Output channel for mercymod logs. The bot must be able to create webhooks in it.')
    async def mercymod_enable_logging(self, ctx: GuildContext, *, channel: discord.TextChannel):
        """Enables all logging in the given channel.

        The bot must have the ability to create webhooks in the given channel.
        """
        await ctx.defer()
        config = await self.get_guild_config(ctx.guild.id)
        if config and config.automod_flags.edits and config.automod_flags.deletes and config.automod_flags.joins:
            await ctx.send(
                f'You already have logging enabled. To disable, use "{ctx.prefix}Mercymod disable"'
            )
            return

        channel_id = channel.id

        reason = f'{ctx.author} (ID: {ctx.author.id}) enabled Mercymod message logs'

        try:
            webhook = await channel.create_webhook(name='Mercymod Logs', avatar=self._avatar, reason=reason)
        except discord.Forbidden:
        ...
        await ctx.send(f'Logs enabled. Broadcasting log messages to <#{channel_id}>.')

    @mercymod.command(name='joins')
    @checks.is_mod()
    @app_commands.describe(
        channel='Output channel join messages. The bot must be able to create webhooks in it.'
    )
    async def mercymod_joins(self, ctx: GuildContext, *, channel: discord.TextChannel):
        ...
        if config and config.automod_flags.joins:
            await ctx.send(
                f'You already have join message logging enabled. To disable, use "{ctx.prefix}mercymod disable joins"'
            )
            return

        channel_id = channel.id

        reason = f'{ctx.author} (ID: {ctx.author.id}) enabled Mercymod join logs'
    ...
    async def migrate_automod_broadcast(self, user: discord.abc.User, channel: discord.TextChannel, guild_id: int) -> None:
        reason = f'{user} (ID: {user.id}) migrated Mercymod join logs'
    ...
    @mercymod.command(name='edits')
    @checks.is_mod()
    @app_commands.describe(
        channel='The channel to broadcast edit messages to. The bot must be able to create webhooks in it.'
    )
    async def mercymod_edits(self, ctx: GuildContext, *, channel: discord.TextChannel):
        """Enables edit message logging in the given channel.
        ...
        await ctx.defer()
        config = await self.get_guild_config(ctx.guild.id)
        if config and config.automod_flags.edits:
            await ctx.send(
                f'You already have edited message logging enabled. To disable, use "{ctx.prefix}Mercymod disable edits"'
            )
            return

        channel_id = channel.id

        reason = f'{ctx.author} (ID: {ctx.author.id}) enabled Mercymod edited message logs'
        ...
        await ctx.db.execute(query, ctx.guild.id, flags.value, channel_id, webhook.url)
        self.get_guild_config.invalidate(self, ctx.guild.id)
        await ctx.send(f'Edit logs enabled. Broadcasting edited messages to <#{channel_id}>.')

    async def disable_automod_edits(self, guild_id: int):
        # Note: This is called when the webhook has been deleted
        query = """INSERT INTO guild_mod_config (id, automod_flags, broadcast_channel, broadcast_webhook_url)
                   VALUES ($1, 0, NULL, NULL) ON CONFLICT (id)
                   DO UPDATE SET
        ...
        await self.bot.pool.execute(query, guild_id, AutoModFlags.edits.flag)
        self.get_guild_config.invalidate(self, guild_id)

    async def migrate_automod_edits(self, user: discord.abc.User, channel: discord.TextChannel, guild_id: int) -> None:
        reason = f'{user} (ID: {user.id}) migrated Mercymod edits logs'

        config = await self.get_guild_config(guild_id)
        if config and config.automod_flags.edits == True:
            # If someone's successfully migrated somehow, just return early
            # The message will hopefully edit.
            return
    ...
    @mercymod.command(name='deletes')
    @checks.is_mod()
    @app_commands.describe(
        channel='The channel to broadcast edit messages to. The bot must be able to create webhooks in it.'
    )
    async def mercymod_deletes(self, ctx: GuildContext, *, channel: discord.TextChannel):
        """Enables edit message logging in the given channel.
        ...
        await ctx.defer()
        config = await self.get_guild_config(ctx.guild.id)
        if config and config.automod_flags.deletes:
            await ctx.send(
                f'You already have deleted message logging enabled. To disable, use "{ctx.prefix}mercymod disable deletes"'
            )
            return

        channel_id = channel.id

        reason = f'{ctx.author} (ID: {ctx.author.id}) enabled Mercymod deleted message logs'
        ...
        await ctx.db.execute(query, ctx.guild.id, flags.value, channel_id, webhook.url)
        self.get_guild_config.invalidate(self, ctx.guild.id)
        await ctx.send(f'Deleted logs enabled. Broadcasting Deleted messages to <#{channel_id}>.')

    async def disable_automod_deletes(self, guild_id: int):
        # Note: This is called when the webhook has been deleted
        query = """INSERT INTO guild_mod_config (id, automod_flags, broadcast_channel, broadcast_webhook_url)
                   VALUES ($1, 0, NULL, NULL) ON CONFLICT (id)
                   DO UPDATE SET
    ...
    async def migrate_automod_deletes(self, user: discord.abc.User, channel: discord.TextChannel, guild_id: int) -> None:
        reason = f'{user} (ID: {user.id}) migrated Mercymod delets logs'
    ...
    async def mercymod_alerts(self, ctx: GuildContext, *, channel: discord.TextChannel):
        ...
        if config and config.automod_flags.alerts:
            await ctx.send(
                f'You already have alert message logging enabled. To disable, use "{ctx.prefix}Mercymod disable alerts"'
            )
            return

        channel_id = channel.id

        reason = f'{ctx.author} (ID: {ctx.author.id}) enabled Mercymod alert message logging'

        try:
            webhook = await channel.create_webhook(name='Mercymod Alerts', avatar=self._avatar, reason=reason)
        except discord.Forbidden:
            await ctx.send(f'The bot does not have permissions to create webhooks in {channel.mention}.')
            return
        except discord.HTTPException:
    ...
    @mercymod.command(name='disable', aliases=['off'])
    @checks.is_mod()
    @app_commands.describe(protection='The protection to disable')
    @app_commands.choices(
        protection=[
            app_commands.Choice(name='Everything', value='all'),
            app_commands.Choice(name='All logging', value='all_log'),
            app_commands.Choice(name='Join logging', value='joins'),
            app_commands.Choice(name='Edit logging', value='edits'),
            app_commands.Choice(name='Delete logging', value='deletes'),
    ...
    async def mercymod_disable(
        self, ctx: GuildContext, *, protection: Literal['all', 'joins', 'edit', 'delete', 'alerts', 'raid', 'mentions', 'gatekeeper'] = 'all'
    ):
        """Disables Mercymod on the server.

        This can be one of these settings:

        - "all" to disable everything
        - "joins" to disable join logging
        - "edits" to disable edit logging
        - "deletes" to disable delete logging
        ...
        if protection == 'all':
            ...
            message = 'Mercymod has been disabled.'
        elif protection == 'joins':
            updates = f'automod_flags = guild_mod_config.automod_flags & ~{AutoModFlags.joins.flag}'
            message = 'Join logs have been disabled.'
        elif protection == 'edits':
            updates = f'automod_flags = guild_mod_config.automod_flags & ~{AutoModFlags.edits.flag}'
            message = 'Edit logs have been disabled.'
        elif protection == 'deletes':
            updates = f'automod_flags = guild_mod_config.automod_flags & ~{AutoModFlags.deletes.flag}'
            message = 'Delete logs have been disabled.'
    ...
    @mercymod.command(name='gatekeeper')
    @checks.is_mod()
    async def mercymod_gatekeeper(self, ctx: GuildContext):
    ...
    @mercymod.command(name='mentions')
    @commands.guild_only()
    @checks.is_mod()
    @app_commands.describe(count='The maximum amount of mentions before banning.')
    async def mercymod_mentions(self, ctx: GuildContext, count: commands.Range[int, 3]):
    ...
    @mercymod_mentions.error
    async def mercymod_mentions_error(self, ctx: GuildContext, error: commands.CommandError):
        if isinstance(error, commands.RangeError):
            await ctx.send('\N{NO ENTRY SIGN} Mention spam protection threshold must be greater than three.')

    @mercymod.command(name='ignore')
    @commands.guild_only()
    ...
    async def mercymod_ignore(
        self, ctx: GuildContext, entities: Annotated[List[IgnoreableEntity], commands.Greedy[IgnoreEntity]]
    ):
        """Specifies what roles, members, or channels ignore Mercymod auto-bans.
    ...
    @mercymod.command(name='unignore')
    @commands.guild_only()
    @checks.hybrid_permissions_check(ban_members=True)
    @app_commands.describe(entities='Space separated list of roles, members, or channels to take off the ignore list')
    async def mercymod_unignore(
        self, ctx: GuildContext, entities: Annotated[List[IgnoreableEntity], commands.Greedy[IgnoreEntity]]
    ):
        """Specifies what roles, members, or channels to take off the Mercymod ignore list.
    ...
    @mercymod.command(name='ignored')
    @commands.guild_only()
    async def mercymod_ignored(self, ctx: GuildContext):
        """Lists what channels, roles, and members are in the Mod ignore list"""
    ...
    @checks.is_mod()
    async def modlog(self, ctx: GuildContext):
    ...
    async def disable_modlog_enable(self, guild_id):
        query = """INSERT INTO guild_mod_config (id, modlog_enable, modlog_chid)
                   VALUES ($1, NULL, NULL) ON CONFLICT (id)
                   DO UPDATE SET
                        modlog_enable = NULL,
                        modlog_chid = NULL;
                """
